          ...
          <table className="stable">
            ...
            <tbody>
              {isLoading ? (
                ...
                data.map((row, i) => (
                  <tr key={row.id}>
                    <td data-label="S.NO" style={{ textAlign: "center", fontWeight: "500", color: "var(--muted)" }}>{String(i + 1).padStart(2, '0')}</td>
                    <td data-label="DEAL ID">{row.deal_id || "-"}</td>
                    <td data-label="BIDDER">{row.bidder || "-"}</td>
                    <td data-label="AUCTION SOURCE">{row.auction_source || "-"}</td>
                    <td data-label="NOTIFICATION DATE">{formatDate(row.notification_date)}</td>
                    <td data-label="BID DATE">{formatDate(row.bid_date)}</td>
                    <td data-label="COAL COMPANY">{row.coal_company || "-"}</td>
