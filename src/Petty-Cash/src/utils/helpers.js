...
export const calculateBalance = (personName, credits = [], expenses = []) => {
  ...
  const targetName = String(personName).trim().toLowerCase();
  const creditAmount = (credits || [])
    .filter(c => {
      const name = String(c.person_name || c.personName || '').trim().toLowerCase();
  ...
  const expenseAmount = (expenses || [])
    .filter(e => {
      const name = String(e.person_name || e.personName || '').trim().toLowerCase();
      return name === targetName && e.status === 'APPROVED';
    })
