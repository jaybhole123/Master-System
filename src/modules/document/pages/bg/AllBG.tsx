...
const AllBG = () => {
    ...
    const [isLoading, setIsLoading] = useState(() => !bgs || bgs.length === 0);
    ...
    useEffect(() => {
        setTitle('All BG');
        loadBGs(false); // Background update, no spinner if data already exists
    }, [setTitle]);

    const loadBGs = async (force: boolean = false) => {
        try {
            if (force || !bgs || bgs.length === 0) {
                setIsLoading(true);
            }
