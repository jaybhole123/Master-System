...
const AllLoans = () => {
    ...
    const [isLoading, setIsLoading] = useState(() => !loans || loans.length === 0);
    ...
    useEffect(() => {
        setTitle('All Loans');
        loadLoans(false); // Background update, no spinner if data already exists
    }, [setTitle]);

    const loadLoans = async (force: boolean = false) => {
        try {
            if (force || !loans || loans.length === 0) {
                setIsLoading(true);
            }
                                ...
                                <div key={item.id} className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-all duration-300">
                                    ...
                                    <div className="grid grid-cols-2 gap-4 mb-5 p-4 bg-gray-50/50 rounded-xl border border-gray-50">
                                        ...
                                        <div className="space-y-1">
                                            ...
                                            <p className="font-bold text-red-600 text-base">{formatCurrency(item.emi)}</p>
                                        </div>
                                        <div className="space-y-1">
